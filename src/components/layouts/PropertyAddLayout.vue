...
const getNextRoute = currPath => {
  return `/auth/signup/${nextPath[currPath]}`
}
...
<template>
  <div class="PropertyAddLayout">
    <div class="propertyAdd-page-number" v-if="isShow !== 'showDone'">
      {{ page }}<span class="total-page"> / 6</span>
    </div>
    <div class="propertyAdd-title-wrapper" v-if="isShow !== 'showDone'">
      <p class="propertyAdd-title-text">{{ title }}</p>
      <p class="propertyAdd-sub-title-text">{{ subTitle }}</p>
    ...
    <div class="propertyAdd-done-wrapper" v-if="isShow === 'showDone'">
      <p>{{ title }}</p>
      <p>{{ subTitle }}</p>
...
.propertyAdd-done-wrapper>p:nth-child(1) {
  font-size: var(--title-size);
  font-weight: var(--font-weight-semibold);
  color: var(--title-text);
}
